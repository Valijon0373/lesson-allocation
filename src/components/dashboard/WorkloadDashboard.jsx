  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans p-6 pb-20">
      <div className="mx-auto w-full space-y-6">
        ...
        {/* O'qituvchilar yuklamasi Section */}
        <div className="mt-10 mb-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
          {/* Navbar-like Header */}
            ...
            <table className="w-full text-left text-sm whitespace-nowrap relative">
              <thead className="bg-slate-50/80 text-slate-500 border-b border-slate-100 sticky top-0 z-10 shadow-sm backdrop-blur-sm">
                <tr>
                  ...
                  <th className="py-3 px-4 font-medium"><div className="flex items-center gap-1 cursor-pointer hover:text-slate-700">O'qituvchi <ArrowUpDown className="w-3 h-3" /></div></th>
                  ...
                  <th className="py-3 px-4 font-medium text-right"><div className="flex items-center justify-end gap-1 cursor-pointer hover:text-slate-700">Jami <ArrowUpDown className="w-3 h-3" /></div></th>
                ...
                {teachersWorkloadData.map((row, idx) => (
                  <tr key={row.id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="py-3 px-4 font-medium text-slate-800">
                      <div className="max-w-[200px] truncate" title={row.name}>{row.name}</div>
                    </td>
                    ...
                    <td className="py-3 px-4 text-right">
                      <button className="p-1 text-slate-400 hover:text-slate-700 transition-colors opacity-0 group-hover:opacity-100">
            ...
            <div className="flex-1 w-full h-[250px] min-h-[250px] flex items-center justify-center relative overflow-hidden">
              ...
              {activeTab === "Fakultet" && (
                <div className="w-full h-full vertical-wipe">
                  <BarChart
                    xAxis={[{ scaleType: "band", data: fakultetData.map((d) => d.name) }]}
                    series={[{ data: fakultetData.map((d) => d.value), color: "#3b82f6" }]}
                    height={250}
              ...
              {activeTab === "Kafedra" && (
                <div className="w-full h-full vertical-wipe">
                  <BarChart
                    xAxis={[{ scaleType: "band", data: kafedraData.map((d) => d.name) }]}
                    series={[{ data: kafedraData.map((d) => d.value), color: "#8b5cf6" }]}
                    height={250}
                    margin={{ top: 10, bottom: 30, left: 40, right: 10 }}
                  />
                </div>
              ...
              {activeTab === "O'qituvchi" && (
                <div className="w-full h-full horizontal-wipe">
                  <BarChart
                    layout="horizontal"
                    yAxis={[{ scaleType: "band", data: oqituvchiData.map((d) => d.name) }]}
                    series={[{ data: oqituvchiData.map((d) => d.value), color: "#10b981" }]}
                    height={250}
